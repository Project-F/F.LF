...
(function (exports) {
  ...
  Unpacker.prototype.unpack_string = function (size) {
    ...
    let i = 0; let str = ''; let c; let code
    while (i < size) {
      c = bytes[i]
      if (c < 128) {
 ...
 */
  ...
  // TODO: fail gracefully.
    ...
    // FIXME: String stuff makes things terribly async.
      ...
      if (msg._multiple) {
        for (let i = 0, ii = msg.length; i < ii; i += 1) {
  ...
  Reliable.prototype._processAcks = function () {
    for (const id in this._outgoing) {
  ...
  // Handle various message types.
    ...
    this._dc.onmessage = function (e) {
      let msg = e.data
      const datatype = msg.constructor
      // FIXME: msg is String until binary is supported.
      // Once that happens, this will have to be smarter.
      if (datatype === String) {
        const ab = util.binaryStringToArrayBuffer(msg)
        msg = util.unpack(ab)
  ...
  Reliable.prototype._handleMessage = function (msg) {
    const id = msg[1]
    const idata = this._incoming[id]
    const odata = this._outgoing[id]
    let data
    switch (msg[0]) {
  ...
  Reliable.prototype._chunk = function (bl) {
    ...
    let start = 0
    while (start < size) {
      const end = Math.min(size, start + this._mtu)
  ...
  Reliable.prototype._ack = function (id) {
    const ack = this._incoming[id].ack
  ...
  Reliable.prototype._calculateNextAck = function (id) {
    const data = this._incoming[id]
    const chunks = data.chunks
    for (let i = 0, ii = chunks.length; i < ii; i += 1) {
  ...
  Reliable.prototype._sendWindowedChunks = function (id) {
    ...
    const data = this._outgoing[id]
    const ch = data.chunks
    const chunks = []
    const limit = Math.min(data.ack + this._window, ch.length)
    for (let i = data.ack; i < limit; i += 1) {
  ...
  Reliable.prototype._complete = function (id) {
    ...
    const self = this
    const chunks = this._incoming[id].chunks
    const bl = new Blob(chunks)
  ...
  // See RFC for more info: http://www.ietf.org/rfc/rfc2327.txt
    ...
    // way.
    let version = navigator.appVersion.match(/Chrome\/(.*?) /)
    if (version) {
  ...
  var util = {
    ...
    supports: (function () {
      ...
      // sync default check.
        ...
        const negotiationDC = negotiationPC.createDataChannel('_PEERJSNEGOTIATIONTEST')
 ...
 */
  ...
  Peer.prototype._retrieveId = function (cb) {
    ...
    let url = this.options.host + '/' + this.options.key + '/id'
 ...
 */
  ...
  DataConnection.prototype._sendChunks = function (blob) {
    ...
    for (let i = 0, ii = blobs.length; i < ii; i += 1) {
      var blob = blobs[i]
      this.send(blob, true)
    }
  }
