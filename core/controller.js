define(function () {
  ...
  controller.prototype.destroy = function () {
    const ii = master_controller.child.indexOf(this)
    if (ii !== -1) { master_controller.child.splice(ii, 1) }
  }
  ...
  controller.prototype.key = function (e, down) // interface to master_controller
  {
    ...
    for (const I in this.config) {
      if (this.keycode[I] == e) {
        if (this.sync === false) {
          if (this.child) {
            for (const J in this.child) { this.child[J].key(I, down) }
          }
          this.state[I] = down
        } else {
  ...
  controller.prototype.clear_states = function () {
    for (const I in this.config) { this.state[I] = 0 }
  }
  /*\
   * controller.fetch
  ...
  controller.prototype.fetch = function () {
    for (let i = 0; i < this.buf.length; i++) {
      ...
      if (this.child) {
        for (let j = 0; j < this.child.length; j++) { this.child[j].key(I, down) }
      }
      this.state[I] = down
    }
    this.buf.length = 0
