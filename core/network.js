  ...
  function frame() // timer frame
  {
    if (This.timer_callback) {
      if (This.frame.buffer[0]) {
        ...
        {
          if (This.frame.buffer[0].time !== This.time) { This.monitor.on('sync_error') }
          This.time++
          const result = This.timer_callback(This.frame.buffer[0].time, This.frame.buffer[0].data, channels.frame.send)
