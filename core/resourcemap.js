...
define(['core/util'], function (Futil) {
  ...
  submap.prototype.update_condition = function () {
    if (this.map.condition) { this.enable = this.map.condition() || false } else { this.enable = true }
    if (typeof this.map.resource !== 'object' &&
      typeof this.map.get !== 'function') { this.enable = false }
  }
  submap.prototype.get = function (res) {
    if (this.enable) {
      if (this.map.resource && this.map.resource[res]) { return this.map.resource[res] } else {
        const url = this.map.get && this.map.get(res)
        if (url) return url
      }
    }
    return null
